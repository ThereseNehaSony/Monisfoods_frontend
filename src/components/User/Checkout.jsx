...
//         toast.success(`Coupon applied! ₹${discountAmount} will be added to wallet upon booking.`);
...
const Checkout = () => {
  ...
  const studentName = location.state?.studentName || 'Guest';
  ...
  const calculateTotal = (discountAmount = discount) => {
    let total = 0;
    ...
    if (isWalletUsed) {
      return Math.max(0, total - walletBalance);
    }
  ...
  const handleApplyCoupon = async () => {
    try {
      ...
      setTotalAmount(calculateTotal(discountAmount));
      toast.success(`Coupon applied! ₹${discountAmount} discount applied.`);
    } catch (error) {
      toast.error('Invalid coupon or failed to apply.');
    }
  };
  ...
  const handleBooking = async (paymentDetails = null) => {
    const formattedItems = Object.keys(selectedItems).reduce((acc, mealType) => {
      acc[mealType] = Object.keys(selectedItems[mealType] || {}).map((itemName) => ({
        name: itemName,
        details: selectedItems[mealType][itemName],
      }));
      return acc;
    }, {});
    ...
    try {
      await axios.post(`${baseURL}/api/user/bookings`, bookingData);
      toast.success('Booking successful!');
      navigate('/payment-success');
    } catch (error) {
      toast.error('Failed to create booking.');
  ...
  useEffect(() => {
    setTotalAmount(calculateTotal());
  }, [selectedItems, discount, isWalletUsed]);

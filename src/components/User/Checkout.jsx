...
const calculateTotal = () => {
  ...
  const handleApplyCoupon = async () => {
    try {
      const response = await axios.post("http://localhost:3000/api/admin/coupons/validate", {
        code: couponCode,
      });
  ...
  const handleShowCoupons = async () => {
    try {
      const response = await axios.get("http://localhost:3000/api/admin/coupons");
  ...
  useEffect(() => {
    const fetchWalletBalance = async () => {
      try {
        const response = await axios.get(`http://localhost:3000/api/user/wallet/balance?userId=${user}`); 
  ...
  const handleBooking = async (paymentDetails) => {
    ...
    try {
      const response = await axios.post('http://localhost:3000/api/user/bookings', bookingData);
  ...
  console.log(totalAmount);
      ...
      const response = await axios.post('http://localhost:3000/api/user/create-order', {
        amount: totalAmount
      });

...
const OrderHistoryPage = () => {
  ...
  const handleCancel = async (orderId, mealType, itemIndex) => {
    try {
      const response = await axios.post(`${baseURL}/api/user/cancel-meal`, {
        orderId,
        mealType,
        itemIndex,
      });
      ...
      const updatedOrders = orders.map((order) => {
        if (order._id === orderId) {
          const updatedItems = { ...order.selectedItems };
          updatedItems[mealType][itemIndex].canceled = true;
          return { ...order, selectedItems: updatedItems };
        }
        return order;
      });
      setOrders(updatedOrders);
    } catch (error) {
      alert('Failed to cancel the meal. Please try again.');
    }
  };
                ...
                <div className="mt-4">
                  {Object.keys(order.selectedItems).map((mealType) => (
                    <div key={mealType}>
                      <h4 className="text-xl font-semibold text-white">{mealType.charAt(0).toUpperCase() + mealType.slice(1)}</h4>
                      {order.selectedItems[mealType].map((item, index) => (
                        <div key={index} className="flex justify-between items-center text-white mt-2">
                          <p>
                            {item.name} ({item.details.size}) - {item.details.quantity} x ₹{item.details.price}
                          </p>
                          {item.canceled ? (
                            ...
                            <button
                              ...
                              onClick={() => handleCancel(order._id, mealType, index)}
                            >
                              Cancel
                            </button>
                          )}
